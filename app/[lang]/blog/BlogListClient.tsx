...
export default function BlogListClient({ initialPosts, initialStatus, lang }: BlogListClientProps) {
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                            {currentPosts.map((post) => (
                                <article key={post.id} className="bg-white dark:bg-gray-900 rounded-sm shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden group hover:shadow-xl transition-all duration-300 flex flex-col h-full">
                                    <Link href={`/${lang}/blog/${post.slug}`} className="relative h-48 overflow-hidden block">
                                        <OptimizedImage
                                            src={post.coverImage}
                                            alt={post.title}
                                    ...
                                    <div className="p-6 flex flex-col flex-grow">
                                        <div className="flex items-center gap-4 text-xs text-gray-500 dark:text-gray-400 mb-4">
                                            <div className="flex items-center gap-1">
                                                <Calendar size={14} />
                                                {new Date(post.date).toLocaleDateString(lang === 'pt' ? 'pt-BR' : lang === 'en' ? 'en-US' : 'es-ES', { month: 'long', day: 'numeric', year: 'numeric' })}
                                            </div>
                                            {post.author && (
                                                <div className="flex items-center gap-1">
                                                    <User size={14} />
