# Light Frosted Glass — `/contacto` Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Apply Light Frosted Glass to `app/contacto/page.tsx` — white canvas with pastel orbes, frosted glass cards for the contact info column and the form column.

**Architecture:** Single file edit. `LeadForm` component is NOT modified — it stays as-is and gets wrapped in a glass container. Icon containers switch from bg-puka-beige/bg-gray-100 to frosted glass. All dark mode classes removed.

**Tech Stack:** Next.js 15, React 19, TypeScript, Tailwind CSS, inline styles for rgba glass values
...
| File | Change |
|------|--------|
| `app/contacto/page.tsx` | Apply Light Frosted Glass — white canvas, orbes, glass cards |

**Do NOT modify:** `components/LeadForm.tsx`, `app/contacto/layout.tsx`, SEO, analytics, i18n logic.

---

### Task 1: Replace `app/contacto/page.tsx` with Light Frosted Glass version

**Files:**
- Modify: `app/contacto/page.tsx`

- [ ] **Step 1: Read current file** to understand exact import list and structure before editing.

Read `app/contacto/page.tsx` — already done in planning. Confirm imports: `React`, `Mail`, `MessageCircle`, `MapPin`, `Clock`, `LeadForm`, `SEO`, `useTranslation`, `ga`.

- [ ] **Step 2: Replace the file content**

Replace the entire `app/contacto/page.tsx` with:
...
```bash
cd /Users/luisviteri/PukaDigital && npx tsc --noEmit 2>&1 | head -30
```

Expected: no errors in `app/contacto/page.tsx`. Pre-existing errors in `proxy.ts` or `types/` are OK to ignore.

- [ ] **Step 4: Commit**

```bash
git add app/contacto/page.tsx
git commit -m "feat(contacto): light frosted glass — canvas blanco, orbes pastel, glass cards"
```
