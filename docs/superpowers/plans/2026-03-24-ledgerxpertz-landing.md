# LedgerXpertz Landing Page Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Construir la landing page definitiva de LedgerXpertz en `app/ledgerxpertz/` con Dark Glass Rojo, Lucide React, precios reales ($10/$15/$20), toggle mensual/anual y SEO completo.

**Architecture:** Una sola página (`page.tsx`) con estado para el toggle de precios, layout separado (`layout.tsx`) para metadata Next.js. Todos los CTAs abren WhatsApp en nueva pestaña con mensajes pre-escritos y tracking de analytics. Los estilos glass se implementan con inline styles (los valores rgba exactos no están disponibles como clases Tailwind estándar).

**Tech Stack:** Next.js 15 App Router, React 19, TypeScript, Tailwind CSS (utilidades base), Lucide React, `@/lib/analytics` para tracking, `@/components/SEO` para schema.org.
...
|---|---|---|
| `app/ledgerxpertz/layout.tsx` | Crear | Metadata Next.js: title, description, OG tags, canonical, keywords |
| `app/ledgerxpertz/page.tsx` | Reemplazar completo | Landing page: Navbar, Hero, Social Proof, Features, Pricing, Comparison, CTA final |
| `app/sistema/` | Eliminar directorio | Código muerto — redirect 301 ya activo en next.config.ts |

**Archivos ya completados** (no tocar):
- `next.config.ts` — redirects 301 activos
- `app/sitemap.ts` — `/ledgerxpertz` con priority 0.9
- `app/inventario/`, `app/sistema-inventario/`, `app/sistema-erp-cloud/` — eliminados

---

## Task 1: Crear `app/ledgerxpertz/layout.tsx`

**Files:**
- Create: `app/ledgerxpertz/layout.tsx`

- [ ] **Step 1: Crear el archivo layout.tsx**
...
```bash
cd /Users/luisviteri/Proyectos/pukadigital && npx tsc --noEmit 2>&1 | head -20
```

Esperado: sin errores en `app/ledgerxpertz/layout.tsx`

- [ ] **Step 3: Commit**

```bash
git add app/ledgerxpertz/layout.tsx
git commit -m "feat(ledgerxpertz): add layout.tsx with full SEO metadata and OG tags"
```

---

## Task 2: Reemplazar `app/ledgerxpertz/page.tsx`

**Files:**
- Modify: `app/ledgerxpertz/page.tsx` (reemplazar completo)

Esta es la landing page completa. Se estructura en un solo componente con secciones claramente delimitadas por comentarios.

- [ ] **Step 1: Reemplazar page.tsx con la implementación completa**
  ...
  return (
    ...
    >
      ...
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 50, ...glass.nav }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '14px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div
              style={{ width: '8px', height: '8px', background: '#C7171E', borderRadius: '50%', boxShadow: '0 0 12px rgba(199,23,30,1)', animation: 'pulse 2s infinite' }}
            />
      ...
      <footer style={{ borderTop: '1px solid rgba(255,255,255,0.06)', padding: '24px', textAlign: 'center', color: 'rgba(255,255,255,0.20)', fontSize: '12px' }}>
        © {new Date().getFullYear()} LedgerXpertz — Powered by PukaDigital
      </footer>
    </div>
  );
}
...
```bash
cd /Users/luisviteri/Proyectos/pukadigital && npx tsc --noEmit 2>&1 | grep -E "ledgerxpertz|error" | head -20
```

Esperado: sin errores en `app/ledgerxpertz/page.tsx`

- [ ] **Step 3: Levantar el servidor de desarrollo y verificar visualmente**

```bash
cd /Users/luisviteri/Proyectos/pukadigital && npm run dev
```

Abrir `http://localhost:3000/ledgerxpertz` y verificar:
- Fondo `#080808` con orbes de luz roja visibles
- Navbar sticky con glass frosted
- Hero con H1 blanco + H1 rojo con glow
- Badge de precio "desde $10/mes" con efecto glass
- Botones CTA funcionando (abren WhatsApp)
- Social proof bar con 4 métricas
- 6 features cards con iconos Lucide
- Toggle mensual/anual funcional (cambia precios y muestra tachado)
- 3 pricing cards con Grow destacado en rojo
- Tabla comparativa
- CTA final con botón verde WhatsApp

- [ ] **Step 4: Verificar redirects desde URLs antiguas**

```bash
curl -I http://localhost:3000/sistema-inventario 2>/dev/null | grep -i location
curl -I http://localhost:3000/sistema-erp-cloud 2>/dev/null | grep -i location
curl -I http://localhost:3000/inventario 2>/dev/null | grep -i location
```

Esperado: cada uno retorna `Location: /ledgerxpertz`

- [ ] **Step 5: Commit**

```bash
git add app/ledgerxpertz/page.tsx
git commit -m "feat(ledgerxpertz): implement full landing page — Dark Glass Rojo, pricing toggle, Lucide icons, WhatsApp CTAs"
```

---

## Task 3: Eliminar directorio `app/sistema/`

**Files:**
- Delete: `app/sistema/page.tsx` y directorio

El directorio `app/sistema/` contiene código muerto — el redirect `'/sistema' → '/ledgerxpertz'` ya está activo en `next.config.ts` y bloquea la ruta.

- [ ] **Step 1: Eliminar el directorio**

```bash
rm -rf /Users/luisviteri/Proyectos/pukadigital/app/sistema
```

- [ ] **Step 2: Verificar que el redirect sigue funcionando**

```bash
curl -I http://localhost:3000/sistema 2>/dev/null | grep -i location
```

Esperado: `Location: /ledgerxpertz`

- [ ] **Step 3: Verificar build de producción limpio**

```bash
cd /Users/luisviteri/Proyectos/pukadigital && npm run build 2>&1 | tail -20
```

Esperado: build exitoso sin errores ni warnings sobre rutas faltantes.

- [ ] **Step 4: Commit final**

```bash
git add -A
git commit -m "chore: remove dead app/sistema/ directory — redirect already active in next.config.ts"
```
...
Después de completar los 3 tasks, verificar:

- [ ] `pukadigital.com/ledgerxpertz` carga la nueva landing
- [ ] `/sistema-inventario`, `/sistema-erp-cloud`, `/inventario`, `/sistema` redirigen a `/ledgerxpertz`
- [ ] El sitemap no incluye URLs legacy
- [ ] El schema.org es válido: pegar el JSON-LD en https://validator.schema.org
- [ ] Los metadata OG están presentes: inspeccionar el HTML del `<head>` en DevTools
- [ ] El toggle mensual/anual muestra precios tachados y equivalentes anuales correctamente
- [ ] Todos los botones abren WhatsApp con el mensaje correcto
- [ ] No hay emojis en ningún lugar — solo iconos Lucide
