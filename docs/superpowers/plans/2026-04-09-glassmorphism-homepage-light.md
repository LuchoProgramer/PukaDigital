# Glassmorphism Light Frosted Glass — Homepage `/` Revision Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Reemplazar el Aurora Dark Glass de `app/page.tsx` por Light Frosted Glass — fondo blanco con cards de vidrio esmerilado, aspecto profesional y claro.

**Architecture:** Un solo archivo modificado. Se reemplaza el objeto `glass` completo y todos los valores de color del canvas oscuro (`#080808`, `white` texts) por la paleta clara. Los orbes pasan de colores intensos a pasteles muy sutiles. Toda la lógica, SEO, analytics y estructura HTML se mantienen intactos.
...
| Archivo | Cambio |
|---------|--------|
| `app/page.tsx` | Reemplazar objeto `glass` + colores del canvas + textos + secciones |

---

### Task 1: Reemplazar `app/page.tsx` completo con Light Frosted Glass

**Files:**
- Modify: `app/page.tsx` (reemplazar archivo completo)

- [ ] **Step 1: Escribir el nuevo `app/page.tsx`**
...
Checklist visual en http://localhost:3000:
- [ ] Fondo blanco con orbes de color apenas perceptibles
- [ ] Header: frosted glass blanco semitransparente, logo visible, botón rojo sólido
- [ ] Hero: texto gris oscuro (#111827), gradiente rojo en el span del h1
- [ ] "El Problema": panel frosted glass sobre fondo levemente rosado
- [ ] Metodología: 3 cards blancas esmeriladas, íconos rojos, hover más blanco con sombra
- [ ] Soluciones: sección con gradiente suave azul-rosa, cards frosted glass, íconos rojos
- [ ] Footer: frosted glass blanco, texto gris suave
...
Esperado: sin errores en `app/page.tsx`.

- [ ] **Step 4: Commit**

```bash
git add app/page.tsx
git commit -m "feat(home): light frosted glass — fondo blanco profesional con glassmorphism"
```
...
**Cobertura:**
- [x] Fondo `#ffffff` con orbes pastel sutiles → Task 1 (wrapper + orbes)
- [x] Header frosted `rgba(255,255,255,0.75)` + `blur(24px)` → Task 1 (header)
- [x] Logo `/logo-Puka.svg` sin cambios → Task 1 (header)
- [x] Botón "Cotizar Ahora" rojo sólido → Task 1 (header)
- [x] h1 `#111827`, h2 `#4B5563` → Task 1 (hero)
- [x] Gradiente rojo en h1 span intacto → Task 1 (hero)
- [x] Badge frosted claro → Task 1 (hero)
- [x] Botón secundario frosted con borde oscuro → Task 1 (hero)
- [x] Panel frosted "El Problema" → Task 1 (sección 2)
- [x] 3 cards frosted metodología + hover → Task 1 (sección 3)
- [x] Íconos rojos en cards → Task 1 (sección 3)
- [x] Sección soluciones gradiente suave → Task 1 (sección 4)
- [x] 2 cards frosted soluciones + hover rojo → Task 1 (sección 4)
- [x] Footer frosted blanco → Task 1 (footer)

**Placeholders:** Ninguno.
**Consistencia:** `hoveredMethod` y `hoveredSolution` definidos y usados en el mismo archivo. Tokens `glass.*` consistentes en todo el archivo.
